...
一個多使用者待辦事項管理應用程式，支援 TED 與 KU 兩位使用者，具備任務建立、篩選、逾期提醒與外部系統連結功能。部署於 Vercel。
...
│   └── ExternalLinks.tsx        # 外部系統連結側欄
└── config/
    └── externalLinks.ts         # 外部連結設定資料
```
...
### 外部連結
- 側欄顯示其他 Sipangzi 系列應用連結（怪獸記帳、小小英雄、英文偵探）
- 桌面版 (lg+) 為右側固定欄，行動版為頂部橫向排列
...
- **行動版**: 外部連結在上方 (order-1)，待辦清單在下方 (order-2)，最大寬度 `max-w-xl`
- **桌面版** (lg: 1024px+): 左側待辦清單 + 右側外部連結側欄 (`w-72`)，flex-row 排列
